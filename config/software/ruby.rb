...
# version_list: url=https://cache.ruby-lang.org/pub/ruby/3.1/ filter=*.tar.gz
# version_list: url=https://cache.ruby-lang.org/pub/ruby/3.2/ filter=*.tar.gz
# version_list: url=https://cache.ruby-lang.org/pub/ruby/3.4/ filter=*.tar.gz
...
build do
  ...
  if windows? && version.satisfies?("~> 3.1.0")
    patch source: "ruby-win32_registry.patch", plevel: 1, env: patch_env
    patch source: "ruby-win32_resolv.patch", plevel: 0, env: patch_env
  end

  if windows? && version.satisfies?("~> 3.4")
    patch source: "ruby-win32_registry_34.patch", plevel: 1, env: patch_env
  ...
  elsif rhel? && platform_version.satisfies?(">= 7") && version.satisfies?(">= 3.1.6") && version.satisfies?("< 3.4.0")
    patch source: "ruby-3.1.6-configure.patch", plevel: 1, env: patch_env
  end
  ...
  #
  if version.satisfies?("~> 3.4.0")
    patch source: "ruby-faster-load_34.patch", plevel: 1, env: patch_env
  end
  # disable libpath in mkmf across all platforms, it trolls omnibus and
  # breaks the postgresql cookbook.  generally you need to have a condition
  # where the embedded and non-embedded libs get into a fight (libiconv,
  # openssl, etc) and ruby trying to set LD_LIBRARY_PATH itself gets it wrong.
  #
  ...
  if fips_mode? &&
    ...
    command "git clone --branch v#{openssl_gem_version} https://github.com/ruby/openssl.git", cwd: "#{install_dir}"
